...
impl FeatureExtractor {
    ...
    pub fn extract(
        ...
        let has_json = content_type.contains("json") || body.starts_with('{') || body.starts_with('[');
        ...
        // Reflection analysis
        let (payload_reflected, reflection_count, reflection_in_attribute, reflection_in_script, reflection_encoded) =
        ...
        let is_api_endpoint = body.starts_with('{') || body.starts_with('[') ||
            response.headers.get("content-type").map(|c| c.contains("json")).unwrap_or(false);
    ...
    fn analyze_reflection(&self, body: &str, payload: Option<&str>) -> (bool, usize, bool, bool, bool) {
        ...
        let in_attribute = body.contains(&format!("=\"{}", payload)) ||
    ...
    fn compare_to_baseline(
        ...
        let differs = status_changed || length_changed_significantly ||
