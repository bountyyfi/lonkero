...
/// Common Firebase paths to test (Realtime Database + common key collections)
const COMMON_PATHS: &[&str] = &[
    // Identity / account data - highest impact for PII exposure
    "/users.json",
    "/user.json",
    "/userprofiles.json",
    "/user_profiles.json",
    ...
    "/feature_flags.json",
    "/features.json",
    // Payment / commerce - financial impact
    "/orders.json",
    "/order.json",
    "/transactions.json",
    "/payments.json",
...
/// Common Firestore collections (REST path: /v1/projects/{id}/databases/(default)/documents/{col})
const FIRESTORE_COLLECTIONS: &[&str] = &[
    ...
    "archive",
    "legacy",
    // KYC / regulated PII – any public-read here is an immediate breach finding.
    "kyc",
    "kyc_documents",
    "kyc_verifications",
    "identity",
    ...
    "insurance",
    "phi",
    // Financial detail beyond plain "payments"
    "bank_accounts",
    "iban",
    "cards",
    "credit_cards",
    ...
    "ledgers",
    "bookkeeping",
    // Communications with high disclosure value
    "private_messages",
    "direct_messages",
    "dms",
    "inbox",
    ...
    // Mobile app collection conventions (Firebase's primary user base)
    "fcm_tokens",
    "device_tokens",
    "devices",
    "push_tokens",
    ...
    "user_secrets",
    "private_user_data",
    // Internal / engineering oversights
    "api_logs",
    "webhook_logs",
    "stripe_webhooks",
    "stripe_customers",
    ...
    "support_tickets",
    "support_messages",
    "feature_flags_internal",
    // Subdocument paths the Firestore REST API also accepts
    "users_private",
    "users-private",
    "private_users",
    "private-users",
...
/// Common Storage prefixes - including environment and sensitive content folders.
///
/// Detection downstream requires either a 200 with `<ListBucketResult>` /
/// `prefixes` JSON, or directly downloadable object listings — so every
/// addition here only increases coverage of paths actually probed, not the
/// likelihood of a false positive flag.
const STORAGE_PREFIXES: &[&str] = &[
    ...
    "licenses/",
    "contracts/",
    // High-impact PII content folders observed in real engagements – each
    // type triggers a regulatory reporting obligation if exposed (GDPR /
    // HIPAA / PCI-DSS / KYC), making any 200 listing immediately actionable.
    "id-cards/",
    "id-card/",
    "ssn/",
    "national-id/",
    ...
    "card-scans/",
    "credit-cards/",
    // Internal / engineering folders that often expose source, secrets,
    // CI artifacts, or full DB dumps.
    "src/",
    "source/",
    "sources/",
    "code/",
    ...
    "vault/",
    "pki/",
    // Backups / DB / log folders – complete data compromise on exposure.
    "db/",
    "database/",
    "databases/",
    "mongo-dump/",
    ...
    "error-logs/",
    "stack-traces/",
    // Mobile-app upload buckets (typical Firebase use case)
    "ios/",
    "android/",
    "mobile/",
    "mobile-uploads/",
...
impl FirebaseScanner {
    ...
    async fn test_firestore(&self, project_id: &str, url: &str) -> (Vec<Vulnerability>, usize) {
        ...
        // Probe a wider slice of collections: the previous cap of 5 left almost
        // all of FIRESTORE_COLLECTIONS unused. Detection still requires a 200 plus
        // a "documents" key in the JSON body, which only Firestore returns for an
        // existing, publicly readable collection — so widening the probe set
        // grows coverage of regulated data (KYC, PHI, financial) without
        // changing the FP profile. We still break on first hit below.
        for collection in FIRESTORE_COLLECTIONS.iter() {
            tests_run += 1;
