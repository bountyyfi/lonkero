...
<param><value><string>http://127.0.0.1/</string></value></param>
    ...
    async fn test_config_exposure(&self, url: &str) -> Vec<Vulnerability> {
        ...
        let config_files = vec![
            ...
            ("/wp-config.php.swo", "Vim recovery file"),
            ...
            // Editor / OS-generated copies that bypass PHP execution
            (".wp-config.php.un~", "Vim undo file"),
            ("/wp-config.php.disabled", "Renamed-disabled config"),
            ("/wp-config.php.dist", "Distribution config"),
            ("/wp-config.php.original", "Renamed original"),
            ("/wp-config.php_old", "Underscore-suffixed backup"),
            ("/wp-config.php_bak", "Underscore-suffixed backup"),
            ("/wp-config.php_backup", "Underscore-suffixed backup"),
            ("/wp-config.inc", "Renamed include"),
            ("/wp-config.inc.bak", "Include backup"),
            ("/wp-config.inc.old", "Old include"),
            ("/wp-config.php.1", "Numbered backup"),
            ("/wp-config.php.2", "Numbered backup"),
            ("/wp-config.php.backup", "Backup with extension"),
            ("/wp-config.php.copy", "Manual copy"),
            // Common archive/compressed copies
            ("/wp-config.zip", "Zipped config"),
            ("/wp-config.tar", "Tarred config"),
            ("/wp-config.tar.gz", "Compressed tar config"),
            ("/wp-config.gz", "Gzipped config"),
            // Hidden folder copies seen on shared hosts
            ("/.wp-config.php", "Hidden config copy"),
            ("/wp-content/wp-config.php", "Misplaced config"),
            ("/wp-includes/wp-config.php", "Misplaced config"),
            ("/wp-admin/wp-config.php", "Misplaced config"),
            ("/wp-admin/setup-config.php", "Setup config (re-install vector)"),
            // Migration / staging tooling artefacts (Duplicator, All-in-One WP Migration, BackWPup)
            ("/installer.php", "Duplicator installer (RCE if not removed)"),
            ("/installer-backup.php", "Duplicator backup installer"),
            ("/installer-data.sql", "Duplicator DB snapshot"),
            ("/installer-log.txt", "Duplicator log with paths/credentials"),
            ("/dup-installer/main.installer.php", "Duplicator pro installer"),
            ("/wp-content/ai1wm-backups/", "All-in-One WP Migration backups"),
            ("/wp-content/backup-db/", "WP-DB-Backup directory"),
            ("/wp-content/backupwordpress-*/", "BackUpWordPress directory"),
            ("/wp-content/uploads/backwpup-*/", "BackWPup directory"),
            ("/wp-snapshots/", "WP Time Capsule snapshots"),
        ];
    ...
    async fn test_debug_log_exposure(&self, url: &str) -> Vec<Vulnerability> {
        ...
        let debug_files = vec![
            ...
            // PHP error logs commonly left by hosts
            "/error.log",
            "/php_errors.log",
            "/php-error.log",
            ...
            "/wp-content/uploads/php-error.log",
            "/wp-admin/error_log",
            "/wp-includes/error_log",
            // WordPress + WooCommerce internal logs
            ...
            "/wp-content/wfcache/",
            "/wp-content/uploads/jetpack-logs/",
            "/wp-content/cache/wp-rocket/",
            "/wp-content/uploads/all-in-one-seo-pack/",
            "/wp-content/uploads/yith-uploads/",
            // Generic CMS / framework log paths attackers always try
    ...
    async fn test_backup_exposure(&self, url: &str) -> Vec<Vulnerability> {
        ...
        let backup_patterns = vec![
            ...
            // Common SQL dump filenames seen in wp-content/ and webroot
            "/dump.sql.gz",
            "/dump.sql.zip",
            "/db.sql.gz",
            "/db.sql.zip",
            ...
            "/test.sql",
            "/local.sql",
            // Domain-name style backups (engineers do this constantly)
            "/wordpress.sql.gz",
            "/wordpress.zip",
            "/wordpress.tar.gz",
            "/site.zip",
            ...
            "/full_backup.zip",
            "/old-site.zip",
            // Plugin backup folders that hold real database snapshots
            ...
            "/wp-content/uploads/dup-installer/",
            "/wp-content/uploads/wpvivid/",
            "/wp-content/uploads/wpvivid_staging/",
            "/wp-content/wpvivid/",
            "/wp-content/uploads/wp-migrate-db/",
            "/wp-content/wp-migrate-db/",
            "/wp-content/ai1wm-backups/",
            "/wp-content/uploads/blogvault/",
            "/wp-content/cache/backupwordpress-*/",
            // Server panel default backup directories
            "/.backup/",
            "/.backups/",
            "/_backup/",
            "/_backups/",
