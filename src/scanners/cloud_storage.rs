...
impl CloudStorageScanner {
    ...
    async fn scan_azure_blob(&self, domain: &str) -> anyhow::Result<(Vec<Vulnerability>, usize)> {
        ...
        // Detection of an exposed account is gated on a 200 with an
        // `<EnumerationResults>`, `<Blobs>`, or `<Containers>` XML body
        // (see detect_exposed_blob()), which is unique to Azure Blob list
        // responses — adding more candidate names here cannot introduce FPs.
        let storage_patterns = vec![
            ...
            mk("functionapp"),
            mk("functionhost"),
            // Default account name shapes that Azure CLI / Bicep / Terraform
            // produce when the project owner doesn't override them.
            mk("st"),
            mk("stor"),
            mk("storprod"),
            mk("stordev"),
    ...
    async fn scan_gcs(&self, domain: &str) -> anyhow::Result<(Vec<Vulnerability>, usize)> {
        ...
        // detect_exposed_gcs() requires a 200 plus `<ListBucketResult>` or a
        // `<Contents>` listing in the body — the same XML schema GCS uses for
        // public-read buckets. Pure 200s without that markup don't match, so
        // additions here only widen the probe set, never relax detection.
        let mut bucket_patterns = vec![
            format!("{}", domain),
            ...
            // Environment / lifecycle expansions
            format!("{}-preprod", domain),
            format!("{}-production", domain),
            format!("{}-qa", domain),
            format!("{}-test", domain),
            format!("{}-uat", domain),
            format!("{}-sandbox", domain),
            format!("{}-internal", domain),
            // High-impact data folders that map to per-bucket layouts in GCS
            format!("{}-credentials", domain),
            format!("{}-keys", domain),
            format!("{}-vault", domain),
            format!("{}-pki", domain),
            format!("{}-confidential", domain),
            format!("{}-documents", domain),
            format!("{}-docs", domain),
            format!("{}-exports", domain),
            format!("{}-export", domain),
            format!("{}-imports", domain),
            format!("{}-reports", domain),
            format!("{}-invoices", domain),
            format!("{}-receipts", domain),
            format!("{}-contracts", domain),
            format!("{}-attachments", domain),
            format!("{}-downloads", domain),
            format!("{}-videos", domain),
            format!("{}-images", domain),
            format!("{}-photos", domain),
            format!("{}-cdn", domain),
            format!("{}-static", domain),
            format!("{}-user-data", domain),
            format!("{}-user-uploads", domain),
            format!("{}-userdata", domain),
            format!("{}-customers", domain),
            format!("{}-customer-data", domain),
            format!("{}-pii", domain),
            format!("{}-kyc", domain),
            // Backup / DB family
            format!("{}-bak", domain),
            format!("{}-snapshot", domain),
            format!("{}-snapshots", domain),
            format!("{}-database", domain),
            format!("{}-database-backups", domain),
            format!("{}-dbbackup", domain),
            format!("{}-mysql", domain),
            format!("{}-postgres", domain),
            format!("{}-mongo", domain),
            format!("{}-mongo-backups", domain),
            format!("{}-bigquery", domain),
            format!("{}-bq-export", domain),
            // CI / IaC / cloudbuild family — leftover artifacts often have full source.
            format!("{}-builds", domain),
            format!("{}-ci", domain),
            format!("{}-cicd", domain),
            format!("{}-pipeline", domain),
            format!("{}-deployments", domain),
            format!("{}-releases", domain),
            format!("{}-terraform-state", domain),
            format!("{}-tf-state", domain),
            format!("{}-tfstates", domain),
            format!("{}-ansible", domain),
            format!("{}-helm", domain),
            format!("{}-charts", domain),
            // Google-default and conventional GCS naming
            format!("{}.firebasestorage.app", domain),
            format!("{}-firebase", domain),
            format!("{}-fb", domain),
            format!("gcf-sources-{}", domain),
            format!("gcf-v2-sources-{}", domain),
            format!("us.artifacts.{}.appspot.com", domain),
            format!("eu.artifacts.{}.appspot.com", domain),
            format!("asia.artifacts.{}.appspot.com", domain),
            format!("{}-cloudfunctions", domain),
            format!("{}-functions", domain),
            format!("{}-gcr", domain),
            // Dataflow / Composer / Vertex AI default buckets
            format!("{}-dataflow", domain),
            format!("{}-composer", domain),
            format!("{}-airflow", domain),
            format!("{}-ml", domain),
            format!("{}-vertex-ai", domain),
            format!("{}-vertex", domain),
            format!("{}-tensorflow", domain),
            format!("{}-models", domain),
            format!("{}-datasets", domain),
            // No-separator variants devs use to satisfy DNS / bucket-naming rules
            format!("{}storage", domain),
            format!("{}backup", domain),
            format!("{}backups", domain),
            format!("{}data", domain),
        ...
        // Drop duplicates that arise when an alias matches an explicit entry,
        // and reject names outside the 3-63 char GCS bucket naming rule.
        let mut seen = std::collections::HashSet::new();
        bucket_patterns.retain(|s| {
            let len = s.len();
            (3..=63).contains(&len) && seen.insert(s.clone())
        });
