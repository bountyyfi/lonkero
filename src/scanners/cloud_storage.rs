...
impl CloudStorageScanner {
    ...
    async fn scan_s3_buckets(&self, url: &str) -> anyhow::Result<(Vec<Vulnerability>, usize)> {
        ...
        let bucket_patterns = vec![
            ...
            format!("{}.data", bucket_base),
            format!("{}.assets", bucket_base),
            // Business / regulated-data themes - highest-impact when exposed
            format!("{}-hr", bucket_base),
            format!("{}-payroll", bucket_base),
            format!("{}-finance", bucket_base),
            format!("{}-financial", bucket_base),
            format!("{}-legal", bucket_base),
            format!("{}-compliance", bucket_base),
            format!("{}-audit", bucket_base),
            format!("{}-audits", bucket_base),
            format!("{}-contracts", bucket_base),
            format!("{}-billing", bucket_base),
            format!("{}-invoicing", bucket_base),
            format!("{}-payments", bucket_base),
            format!("{}-transactions", bucket_base),
            format!("{}-customers", bucket_base),
            format!("{}-clients", bucket_base),
            format!("{}-crm", bucket_base),
            format!("{}-support", bucket_base),
            format!("{}-tickets", bucket_base),
            format!("{}-orders", bucket_base),
            format!("{}-shipments", bucket_base),
            // ML / AI datasets and models
            format!("{}-ml", bucket_base),
            format!("{}-ml-data", bucket_base),
            format!("{}-models", bucket_base),
            format!("{}-training", bucket_base),
            format!("{}-training-data", bucket_base),
            format!("{}-datasets", bucket_base),
            format!("{}-dataset", bucket_base),
            // Analytics / events / tracking
            format!("{}-analytics", bucket_base),
            format!("{}-events", bucket_base),
            format!("{}-metrics", bucket_base),
            format!("{}-tracking", bucket_base),
            format!("{}-warehouse", bucket_base),
            format!("{}-lake", bucket_base),
            format!("{}-datalake", bucket_base),
            format!("{}-data-lake", bucket_base),
            format!("{}-etl", bucket_base),
            // CDN / web / app variants
            format!("{}-cdn", bucket_base),
            format!("{}-www", bucket_base),
            format!("{}-web", bucket_base),
            format!("{}-app", bucket_base),
            format!("{}-mobile", bucket_base),
            format!("{}-api", bucket_base),
            // Common infrastructure themes
            format!("{}-vpc", bucket_base),
            format!("{}-lambda", bucket_base),
            format!("{}-functions", bucket_base),
            format!("{}-serverless", bucket_base),
            format!("{}-s3", bucket_base),
            format!("{}-storage", bucket_base),
            // Year-suffixed buckets - very common for annual archives
            format!("{}-{}", bucket_base, chrono::Utc::now().year()),
            format!("{}-{}", bucket_base, chrono::Utc::now().year() - 1),
            format!("{}-{}", bucket_base, chrono::Utc::now().year() - 2),
            format!("{}{}", bucket_base, chrono::Utc::now().year()),
        ];
