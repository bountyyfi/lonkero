...
const SERVICE_FINGERPRINTS: &[ServiceFingerprint] = &[
    ...
    // Webflow — the body string is unique to a Webflow custom domain that has
    // not been claimed and only renders when the proxy receives a domain it
    // doesn't recognise. Attackers can claim by attaching the domain to a Webflow
    // site they own.
    ServiceFingerprint {
        name: "Webflow",
        ...
        cvss: 8.0,
        confirmed_exploitable: true,
        remediation: "Remove the dangling CNAME or attach the domain to a Webflow site you own.",
    },
    // Strikingly
    ServiceFingerprint {
        name: "Strikingly",
    ...
    // Unbounce — landing-page service; an unclaimed domain returns this exact
    // Apache-style 404 from their edge.
    ServiceFingerprint {
        name: "Unbounce",
        ...
        cvss: 8.0,
        confirmed_exploitable: true,
        remediation: "Remove the dangling CNAME or claim the domain in an Unbounce landing page.",
    },
    ...
    ServiceFingerprint {
        ...
        cvss: 8.0,
        confirmed_exploitable: true,
        remediation: "Remove the dangling CNAME or attach the domain to an Acquia site.",
    },
    // Smartling
    ServiceFingerprint {
        name: "Smartling",
        cname_patterns: &[".smartlingcdn.com"],
        http_signatures: &[
        ...
        cvss: 6.5,
        confirmed_exploitable: true,
        remediation: "Remove the dangling CNAME or configure the domain in Smartling.",
    },
    ...
    ServiceFingerprint {
        ...
        cvss: 7.5,
        confirmed_exploitable: true,
        remediation: "Remove the dangling CNAME or create the ReadMe project for this domain.",
    },
    // Bigcartel storefronts
    ServiceFingerprint {
        name: "Bigcartel",
        ...
        cvss: 7.0,
        confirmed_exploitable: true,
        remediation: "Remove the dangling CNAME or configure the domain in a Bigcartel store.",
    },
    // Wix custom domains
    ServiceFingerprint {
        name: "Wix",
        ...
        cvss: 8.0,
        confirmed_exploitable: true,
        remediation: "Remove the dangling CNAME or connect the domain in a Wix site you control.",
    },
    // Cloudflare Pages — only the exact CNAME pattern combined with the
    // distinctive 522/1016/project body avoids false positives on real,
    // deployed Pages projects.
    ServiceFingerprint {
        ...
        header_patterns: &[("cf-error-code", "1016")],
        nxdomain_vulnerable: false,
        severity: Severity::Medium,
        cvss: 7.0,
        confirmed_exploitable: true,
        remediation: "Remove the dangling CNAME or attach the domain to a Pages project you own.",
    },
    // Render
    ServiceFingerprint {
        name: "Render",
        ...
        cvss: 8.0,
        confirmed_exploitable: true,
        remediation: "Remove the dangling CNAME or attach the domain to a Render service.",
    },
    // Railway
    ServiceFingerprint {
        name: "Railway",
        ...
        cvss: 8.0,
        confirmed_exploitable: true,
        remediation: "Remove the dangling CNAME or attach the domain to a Railway service you own.",
    },
];
...
impl SubdomainTakeoverScanner {
    ...
    async fn enumerate_subdomains(&self, domain: &str, thorough: bool) -> Vec<String> {
        ...
        //
        // The second block adds high-value internal-infrastructure prefixes —
        // service meshes, secret stores, container orchestration, log/metric
        // sinks, out-of-band management — that frequently end up published to
        // production DNS when an internal control plane is misconfigured.
        // Each one we hit is typically a credential/admin-panel exposure, not
        // a marketing surface.
        let extended_prefixes = vec![
            ...
            "logging",
            "logs",
            // Secret stores & service-mesh control planes (typically internal,
            // often credential-bearing if accidentally exposed)
            "vault",
            "consul",
            "nomad",
            "hashicorp",
            ...
            "portainer",
            "kubeapps",
            // Build / artifact pipelines (CI tokens, signed artifacts)
            "nexus",
            "artifactory",
            "jfrog",
            "registry",
            ...
            "bamboo",
            "octopus",
            // Observability stacks (logs/metrics/traces routinely leak PII)
            "grafana",
            "prometheus",
            "alertmanager",
            "thanos",
            ...
            "fluent-bit",
            "vector",
            // Message brokers / streaming (admin panels often unauthenticated)
            "rabbitmq",
            "rabbit",
            "kafka",
            "kafka-ui",
            ...
            "ad",
            "dc",
            // API / docs surfaces (full endpoint enumeration)
            "swagger",
            "apidocs",
            "api-docs",
            "redoc",
            ...
            "playground",
            "explorer",
            // Out-of-band management (game over if Internet-facing)
            "ilo",
            "idrac",
            "ipmi",
            "drac",
            ...
            "kvm",
            "console-server",
            // Networking infra control planes
            "firewall",
            "fortigate",
            "fortinet",
            "sonicwall",
            ...
            "unifi",
            "meraki",
            // Industrial / IoT (often safety-critical)
            "scada",
            "plc",
            "hmi",
            "modbus",
            ...
            "axis",
            "hikvision",
            // Lifecycle ghosts (the host that "no one owns anymore")
            "legacy",
            "archive",
            "archived",
            "deprecated",
            ...
            "old-api",
            "former",
            // Environment qualifiers people forget
            "dev-internal",
            "internal-api",
            "internal-admin",
            "private-api",
