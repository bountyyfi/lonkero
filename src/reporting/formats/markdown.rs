...
impl MarkdownReportGenerator {
    ...
    fn generate_markdown(&self, report: &EnhancedReport, branding: &BrandingConfig) -> String {
        ...
        md.push_str(&format!("**Overall Risk Score:** {:.2}/10.0 ({})\n\n",
        ...
        for (category, vulns) in &report.owasp_mapping {
            md.push_str(&format!("### {}\n\n", category));
            md.push_str(&format!("**Affected Vulnerabilities:** {}\n\n", vulns.len()));
            md.push_str(&format!("{}\n\n",
        ...
        for (idx, vuln) in report.scan_results.vulnerabilities.iter().enumerate() {
            ...
            md.push_str(&format!("**Severity:** {} | **Confidence:** {} | **CVSS:** {:.1}\n\n",
                vuln.severity,
                vuln.confidence,
                vuln.cvss));

            md.push_str(&format!("**CWE:** {} | **Category:** {}\n\n", vuln.cwe, vuln.category));
